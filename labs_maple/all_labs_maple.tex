...
\externaldocument[primer-]{primer_maple}
...
something in the lab sheet like [8.6], that means that item 8.6
...
\end{verbatim}
 ...
 the end [\ref{primer-semi}].  If Maple says ``Warning,
...
\end{verbatim}
 ...
 multiplication[\ref{primer-star}]; it is a very common error to 
...
\begin{exercise}\label{ex-e-digits}
 ...
 [\ref{primer-exp}]).  Read [\ref{primer-evalfdigits},\ref{primer-digits}] 
 to find out how to calculate $4997$ digits of $e$.  (You should notice
...
\begin{exercise}\label{ex-near-int}
 ...
 \begin{itemize}
  ...
  \item[(b)] Enter \verb~x:=exp(Pi*sqrt(163));~.  Maple will
   ...
   [\ref{primer-colonequals},\ref{primer-exp},\ref{primer-pi},\ref{primer-sqrt}]. 
...
\begin{exercise}\label{ex-sin-taylor}
 ...
 $x=4$ (see [\ref{primer-basicplot}] for the syntax).  Now plot
 the function
 \[ y = x-x^3/6+x^5/120-x^7/5040 \]
 from $x=-4$ to $x=4$.  What do you notice?  Plot the two curves
 together, as explained in [\ref{primer-twoplots}].
\end{exercise}
...
\begin{exercise}
 Read [\ref{primer-diff},\ref{primer-diffvar}].  Then ask Maple to
...
\begin{exercise}\label{ex-cubic-horrid}
 ...
 formula.  Read [\ref{primer-fsolve}], and ask Maple to find an
...
\begin{exercise}
 ...
 \begin{itemize}
  \item[(a)] To find the solutions graphically, plot the
   ...
   [\ref{primer-vertrange}], and it is also useful to ask Maple
   to calculate extra points to make the graph more accurate
   [\ref{primer-numpoints}].  You should see that there are many
...
\begin{exercise}\label{ex-linear-trichotomy}
 ...
 using Maple [\ref{primer-solve},\ref{primer-multisolve}].  Note that
...
\begin{exercise}\label{ex-lhopital}
 ...
 (Remember [\ref{primer-arrow}] when entering this in Maple.)
...
Read [\ref{primer-implicitplot}] and ask Maple to plot the
graph. 
...
\begin{exercise}\label{ex-listplot}
 Read [\ref{primer-listplot} --- \ref{primer-listplotgen}].  Ask Maple
...
\begin{exercise}\label{ex-Li}
 ...
 \begin{itemize}
  ...
  \item[(c)] Enter \verb~Q:=(x,h)->(q(x+h)-q(x))/h;~, so that $q'(x)$
   ...
   convenience [\ref{primer-exp}].  Tell Maple to do all numerical
...
\begin{exercise}\label{ex-rolle}
 ...
 \begin{itemize}
  ...
  \item[(b)] Define $f(x)=x^3-x$ using [\ref{primer-arrow}]. 
   ...
   \begin{itemize}
    \item Ask Maple [\ref{primer-roots}] to find the roots of $f(x)$. 
    \item Ask Maple [\ref{primer-diff},\ref{primer-leibniz}] to find
...
\begin{exercise}\label{ex-hermite}
 ...
 definition in Maple [\ref{primer-seriesdef},\ref{primer-multidiff}], and then
