...
\begin{secenum}
 \item\label{help} For help, type \verb~?~ and press
...
%  problems to me.) 
%
 \item\label{semi} Commands must end with a semicolon
...
%
 ...
 of space to print.  
%
 \item\label{percent} You can use the symbol \verb~%~
...
\begin{secenum}
 \item\label{evalf} To get an explicit numerical approximation for an
  expression, use the \verb~evalf()~ function.  For example,
  \verb~evalf(Pi)~ gives $3.141592654$, and
  \verb~evalf(x/3)~ gives $0.3333333333 x$. 
%
 \item\label{evalfdigits} The \verb~evalf()~ function generally gives an
...
%
 \item\label{digits} As an alternative, you can enter
...
%
  \item\label{case} Note that case is significant: Maple regards
...
%
  ...
  for example. 
%
  \item\label{greek} Greek letters can be entered using
  ...
  capital P) not \verb~pi~ (see [\ref{badpi}]). 
%
  \item\label{subscripts} Sometimes one uses subscripted
...
%
 \item\label{powers} Enter powers using the \verb~^~ character,
...
%
 \item\label{expand} Enter \verb~expand(A)~ to expand everything out.  For
...
%
 \item\label{symbolic} If Maple is refusing to simplify $(a^4)^{1/4}$ to $a$ (for
...
%
 \item\label{sort} If Maple writes an expression with the terms in an unnatural
...
%
 \item\label{badpi}
  ...
  as \verb~Pi~. 
%
 \item\label{exp} The constant $e\simeq 2.718281828$ \textbf{cannot} (by default)
...
\begin{secenum}
%
 \item\label{solve} To solve the equation $x^2-5x+6=0$ (for example), enter
...
\end{verbatim}
%
 \item\label{nosols} If there are no solutions, Maple will just give you a new
  prompt without printing any answer at all. 
%
 \item\label{envexplicit} If the answer involves the word
...
%
 \item\label{fsolveinitial}
  ...
  near $x=3$.  
%
 \item\label{froots} As with the \verb~solve()~ command, if you are
...
%
 \item\label{log} Both \verb~ln(x)~ and \verb~log(x)~
  refer to the natural logarithm (to base $e$) of $x$. 
%
 \item\label{logbase}
...
%
 \item\label{expx} The function $e^x$ should be entered as \verb~exp(x)~. 
%
...
%
 \item\label{cosec} In traditional notation $1/\sin(x)$ is sometimes written as
  ...
  enter \verb~1/sin(x)~. 
%
 \item\label{arctan} In traditional notation $\tan^{-1}(x)$ or $\arctan(x)$ refers
...
%
 \item\label{possqrt} When $x\geq 0$, the notation \verb~sqrt(x)~ means, by
...
\begin{secenum}
%
 \item\label{arrow} If you want to define $f(x)=x^2$, enter
...
%
 \item\label{badfuncdef} It \textbf{does not} work very well to enter
...
%
 \item\label{vertrange} To make the vertical axis run from $0$ to $10$ (for
...
%
 \item\label{plotpredef} If the function to be plotted is
...
%
 \item\label{twoplots} To plot the graph $y=x^3+x$ in the same picture, enter
  ...
  between here and [\ref{parametric}] below.)  Similarly,
  to plot $\cos(t)$ and $\sin(t)$ 
  together from $t=-3\pi$ to $t=3\pi$, enter
  \verb~plot([sin(t),cos(t)],t=-3*Pi..3*Pi)~. 
%
 \item\label{discont} To find and skip over discontinuities, add the option
  \verb~discont=true~; for example:
\begin{verbatim}
 plot(frac(x),x=-3..3,discont=true);
\end{verbatim}
%
 \item\label{numpoints} To improve the accuracy of a graph, use the option
...
%
 \item\label{implicitplot} To plot a curve given implicitly, say
...
%
 \item\label{saveplot} To generate and save a plot for future use, enter
  something like this: \\
...
%
 \item\label{diffvar} Note that it is necessary to specify the
...
%
 \item\label{leibniz} In a different kind of traditional notation, you
...
\end{verbatim}
%
 \item\label{inertdiff} Sometimes you just want to mention the derivative
...
%
 \item\label{intconst} Maple's answer will never include an arbitrary
...
\begin{secenum}
%
 \item\label{seq} To generate a sequence of terms, use the \verb~seq()~
...
%
 \item\label{sum} To find the sum of a sequence of terms, use the
...
%
 \item\label{add} Occasionally it works better to use the \verb~add()~
...
\begin{secenum}
%
 \item\label{rememberstar} Remember \verb~*~'s for multiplication.  
  \begin{enumerate}
