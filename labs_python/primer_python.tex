...
\begin{itemize}
 \note{help} If you know the name of a function that you want
...
%
 \note{lastoutput} If you have several commands in the same code cell
...
%
 \note{underscore} You can use the symbol \pcode~_~
...
\begin{itemize}
 \note{float} To get an explicit numerical approximation for an
...
%
 \note{floatdigits} The function \pcode~sp.N()~ generally gives an
...
%
 \note{digitslost} It is possible to lose accuracy for non-obvious
...
\begin{itemize}
 \note{symbols} By default names like $x$ refer to variables as used
...
%
 \note{subscripts} If you want variables 
...
%
  \note{case} Note that case is significant: Python regards
...
%
  \note{greek} If you enter \pcode+mu = sp.symbols('mu')+ then in some
...
\begin{itemize}
 \note{star} Use a \pcode~*~ for multiplication; for example,
...
%
 \note{powers} Enter powers using \pcode~**~,
...
%
 \note{solvesubs}
  ...
  and then \pcode~(x**y + y**2).subs(sol)~.
%
 \note{expand} Enter \pcode~sp.expand(A)~ to expand everything out.  For
...
%
 \note{symbolic} Note that rules like $(a^4)^{1/4}=a$ are
...
\begin{itemize}
 \note{rational} If you enter a fraction like \pcode~2/9~ then it
...
\begin{itemize}
%
 \note{solve} To solve the equation $x^2+6=5x$ (for example), enter
  ...
  \pcode~solve(g(t)=0,t)~ to solve the equation $g(t)=0$. It
...
%
  \note{roots}
   ...
   should instead enter \pcode~sp.solve({x^2-5*x+6=0},dict=True)~.
%
  \note{realroots}
...
\end{verbatim}
%
 \note{nosols} If there are no solutions, \pcode~sp.solve()~ will
...
%
 \note{fsolve} To get approximate numerical solutions, you can 
...
%
 \note{log} Both \pcode~sp.ln(x)~ and \pcode~sp.log(x)~ refer to the natural
...
%
 \note{expx} The function $e^x$ should be entered as \pcode~sp.exp(x)~. 
%
...
%
 \note{arctan} In traditional notation $\tan^{-1}(x)$ or $\arctan(x)$
...
%
 \note{possqrt} When $x\geq 0$, the notation \pcode~sp.sqrt(x)~ means, by
...
\begin{itemize}
%
 \note{arrow} If you want to define a simple function like 
...
%
 \note{basicplot} To plot the graph $y=x^3-x$ from $x=-2$ to $x=2$ (for
  ...
  explicit.
% 
 \note{legend} To label the curve in the above example, we could
...
% 
 \note{linestyle} Various features of the plotted curve can be changed
 ...
 or as hex codes (like
  ...
  required. 
%
 \note{layout} Various features of the overall picture can be changed 
...
%
 \note{scaling} To make the vertical scale the same as the horizontal
  ...
  the notebook.)
%
 \note{twoplots} To plot more than one graph in the same picture, we
  ...
  \end{pcodeblock}
%
 \note{piticks} In the above example, there are tick marks on the
 ...
 \note{hideaxes} To make the $x$ and $y$ axes invisible (together with
 ...
 \note{discont} If we plot a function with discontinuities, like
 ...
 \note{numpoints} To improve the accuracy of a graph, just increase
...
%
 \note{parametric} To plot the curve given parametrically by
  ...
  \end{verbatim}
 \note{implicitplot} Consider a curve given implicitly, say
  ...
  \end{pcodeblock}
%
 \note{saveplot} We might want to produce a basic picture, and then
...
\end{verbatim}
%
 \note{inertdiff} Sometimes we just want to mention the derivative
...
%
  \note{taylorconv} The command \pcode~p = sp.series(...)~ command
...
%
 \note{intconst} Sympy's answer will never include an arbitrary
...
\begin{itemize}
%
 \note{seq} To generate a finite sequence of terms, we can use
 ...
 required. 
%
 \note{sum} For a finite list $u$ as above, we can just enter
...
%
 \note{symbolicsum} However, we might want to get a general formula
...
\begin{itemize}
 \note{symbolsagain} All abstract symbols need to be introduced using somthing
...
%
 \note{rememberstar} Remember \pcode~*~'s for multiplication.  
  \begin{itemize}
...
%
 \note{expbrac} $a^{1/4}$ must be entered as
  \pcode~a^sp.Rational(1,4)~, not \pcode+a^(1/4)+ or \pcode~a^1/4~. 
%
...
%
 \note{removedefs} Remember to remove definitions when you
  ...
  definitions.  You can enter \pcode+del x,y+ to do that.  
%
 \note{badexp} Remember the constant $e=2.71828\dots$ must be
  ...
  \pcode~exp(x)~.  If you enter \pcode+e ** x+ instead of $sp.exp(x)$,
