...
\externaldocument[primer-]{primer_python}
...
Numbers in square brackets refer to the SymPy syntax primer
which you can find on the course home page.
...
\begin{pcodeblock}
  2+2
\end{pcodeblock}
 then press SHIFT-ENTER.  Assuming that everything is set up
 correctly, you should see $4$.  
...
\end{pcodeblock}
 ...
 multiplication [\ref{primer-star}].  

 In both of the examples above, we had only a single calculation in
 each code cell.  We can combine two or more calculations in the same
 code cell, but then only the last one will be printed.  If we want to
 print all of the results, then we need to add explicit print
 statements, like
 \begin{pcodeblock}
  print(2+2)
 ...
 Note that in the second calculation, the answer is reported as $3.0$
 rather than $3$.  This means that Python is treating it as a real
 number whose fractional part happens to be zero, which has a
 different internal representation from the integer $3$.  If we write
 \pcode+n // m+ instead of \pcode+n / m+ then we will always get a
 genuine integer, but this will just discard the remainder, if there
 is one.
\end{exercise}
...
\begin{exercise}\label{ex-numeric-misc}
 ...
 \begin{itemize}
  ...
  \item[(d)] $(10^9+1)(10^{10}-10-9^2)/9^2$ (You should be able to
   ...
   itself.)  
 \end{itemize}
 Again you could write either \pcode+n // m+ or \pcode+n / m+ when
 dividing an integer $n$ by an integer $m$.  You can use the second
 form to convince yourself that the answer is really an integer so
 that there is no remainder, then switch to the first form to get rid
 of the superfluous $.0$.
\end{exercise}

Python will usually convert fractions like $1/7$ to approximate
floating point numbers like $0.142857$.  If you want to work with
exact fractions, you need to enter something like
\pcode+sp.Rational(1, 7)+ instead of $1/7$.  If you have an exact
fraction $x$ then you can enter \pcode+float(x)+ to convert it to a
...
\end{pcodeblock}
 Note here that we only need to use \pcode+sp.Rational()+ for the
 innermost fraction of $1/139$.  As soon as we have one exact
 fraction, Python knows that it should work consistently with exact
 fractions from there onwards.
\end{exercise}

The full story about the precision of floating point arithmetic is
complicated, but it is roughly true to say that calculations are
usually done to 16 significant figures.  If we need more accuracy than
that, we can use the function \pcode+sp.N(x,k)+, which will calculate
$x$ to $k$ significant figures.

\begin{exercise}\label{ex-e-digits}
 Enter \pcode+x = sp.N(sp.exp(1),4997)+ to calculate the number
 $e=\exp(1)\approx 2.718$ to $4997$ digits.  If we just do
 \pcode+print(x)+ then we will see all $4997$ digits on the same line,
 which is inconvenient.  It is better to do the following, to print
 the result on multiple lines with $100$ digits per line:
 \begin{pcodeblock}
  ...
  for i in range(0, len(s), m):
      print(s[i:i+m])
 \end{pcodeblock}
 You will see that the last five digits are $66666$.  This is the
 first place in the decimal expansion of $e$ where a digit is repeated
 five times.
\end{exercise}
...
\begin{exercise}\label{ex-near-int}
 ...
 \begin{itemize}
  \item[(x)] Enter
   \pcode+x = sp.N(sp.exp(sp.pi * sp.sqrt(163)),40)+ (which sets
   $x$ equal to $e^{\pi\sqrt{163}}$) and then \pcode+print(x)+.  You
   should see that there is a long string of nines after the decimal
   point, indicating that $x$ is close to being an integer.
  \item[(c)] Enter \pcode+z = sp.ceiling(x)+, to set $z$ equal to the
   integer closest to $x$.  Then print $z-x$ to see that the
   difference between $x$ and $z$ has absolute value less than
   $10^{-12}$. 
  \item[(d)] Now $x$ and $z$ have numerical values, which will be
   inconvenient later if we want to use them as abstract symbols.  We
   therefore enter \pcode+del x, z+ to remove the values of $x$ and
   $z$. 
 \end{itemize}
\end{exercise}
...
\end{pcodeblock}
 This declares $x$ and $y$ to be abstract symbols, and defines
 $A=(x^2-4y^2)(x^3-xy^2)$.  (Note that in Python $x^n$ is expressed
 as \pcode+x ** n+, not as \pcode+x^n+, and we also need a single star
 for multiplication.)  
 
 Now try the expressions \pcode+A.simplify()+,
 \pcode+A.factor()+, \pcode+A.expand()+ and 
 \pcode+sp.polys.polyfuncs.horner(A)+.  This gives four different ways
 to rewrite the expression $A$, all of which are potentially useful
 for different applications.  Which of them do you prefer?

 There are some things to say about how to display symbolic
 expressions like these.  Generally you should enter
 \pcode+display(A)+, and then you will see $(x^2-4y^2)(x^3-xy^2)$.  If
 you just evaluate $A$ as the last expression in a cell, then it will
 also have the same effect.  However, if you enter \pcode~print(A)~,
 you will instead see \pcode~(x**2 - 4*y**2)*(x**3 - x*y**2)~, which
 is probably not what you want.  You might also want to construct a
 sentence containing some mathematical expressions, which you can do
 like this:
 \begin{pcodeblock}
  display(Latex(
...
\begin{exercise}\label{ex-rat-misc}
 ...
 and then enter \pcode+A.simplify()+ to simplify it.  See if you can
 get the same answer by hand.
\end{exercise}

\section{Plotting}

\begin{exercise}\label{ex-sin-taylor}
 We first want to plot the graph of $y=\sin(x)$ from $x=-4$ to
 $x=4$.  For this we enter
 \begin{pcodeblock}
  ...
  fig.show()
 \end{pcodeblock}
 The first line sets $x$ to be an array $[-4,-3.96,-3.92,\dotsc]$
 consisting of $201$ equally spaced points between $-2$ and $2$
 (inclusive).  The second line sets $y$ to be the array 
 $[\sin(-4),\sin(-3.96),\sin(-3.92),\dotsc]$ obtained by taking the
 sine of each entry in $x$.  The third line creates an object
 called \pcode+fig+ representing a picture, to which we can add
 curves, shapes text and so on.  In this case we just add a single
 curve passing through the points with coordinates $(x_i,y_i)$; this
 is done by the line \pcode+fig.add_trace(go.Scatter(x=x, y=y))+.
 Finally, the line \pcode+fig.show()+ to show the picture that we have
 constructed.  This is not always necessary, if the last line in a
 cell is related to plotting then Python will often decide for itself
 that the resulting plot should be shown, but it does not hurt to make
 it explicit.
 ...
 \end{pcodeblock}
 before \pcode+fig.show()+.  This will plot the function
 \[ y = x-x^3/6+x^5/120-x^7/5040 \]
 along with $y=\sin(x)$, with the two curves in different colours.
 What do you notice?  
\end{exercise}
...
\begin{exercise}
 Ask Python to solve the equations 
 \begin{align*}
  x^2 + y^2 &= 1 \\
  (x-1)^2 + (y-1)^2 &= 1,
 \end{align*}
...
\begin{exercise}
 Read [\ref{primer-diff},\ref{primer-diffvar}].  Then ask Python to differentiate
 the function $\ln(\ln(\ln(x)))$ with respect to $x$ (making sure that
 you first declare $x$ to be an abstract symbol).  Now differentiate
 the functions $(3x+4)/(2x+3)$ and $(1+x^2+x^4/2)e^{-x^2}$, and
 simplify your answers.
\end{exercise}

\begin{exercise}
 Ask Python to evaluate the integral
 $\int_0^1\sqrt{1-x^2}\,dx$, as explained in [\ref{primer-defint}]
\end{exercise}

\chapter{Solving equations} 

Sympy has powerful facilities for solving equations, but they are not
perfect.  Often one has to rearrange an equation in some way before
sympy will succeed in solving it.  It is quite common for sympy to
...
Numbers in square brackets refer to the ``Python
...
\end{pcodeblock}
 ...
 (See [\ref{primer-arrow} -- \ref{primer-showfuncdef}] for more about
 the syntax used here for defining functions.) Then solve 
 $f(x)=0$ (as explained in [\ref{primer-solve}])
 to find the roots.  Use the answer to factorize $f(x)$, and then
 enter \verb~display(sp.factor(f(x)));~ to check your answer.
\end{exercise}

\begin{exercise}\label{ex-biquadratic-roots}
 Declare $x$ to be an abstract symbol, then define
 $y=x^4-x^3-x^2-x/8+1/64$ and $z=16x^3-24x^2-6x+2$.  (Here we do not
 use the lambda notation, because we do not have a $(x)$ on the left
 hand side.  We also need to enter $1/64$ as \verb~sp.Ration1l(1,64)~
 to ensure that it is treated as an exact fraction.)  Ask sympy to
 find the values of $x$ where $y=0$; you should get four solutions.
 It is best to use a command like \verb~sols = sp.solve(y)~ so that
 you can reuse the solutions later; now \verb~sols[0]~ will refer to
 the first solution, \verb~sols[1]~ will refer to the second, and so
 on.  While sympy's solutions are correct, they are not given in the
 simplest possible form.  It turns out that the solutions are given by
 $x=(1\pm\sqrt{2})(1\pm\sqrt{3})/4$.  (There are four possible ways to
 choose the two $\pm$ signs, giving four different solutions.)  Enter
 these four numbers and ask sympy to check that they are the same as
 the solutions that it found earlier.  (Remember that the best way to
 check whether $A=B$ is to simplify $A-B$ and check whether you get
 zero.)

 Next, read [\ref{primer-subs} -- \ref{primer-solvesubs}].  Use the \verb~subs()~
 method to find the value of $z$ at the four points where $y$ is
 zero.  (There are some interesting phenomena going on in this
 calculation, that most naturally explained using Galois Theory.)
\end{exercise}
...
\begin{exercise}\label{ex-cubic-horrid}
 ...
 numbers as an intermediate stage.  For an example of this, ask for
 the roots of $x^3-3x+1$.  Notice that the answer involves $i$ (the
 square root of minus one) in several places.

 In a case like this, it is more useful to find numerical
 approximations to the roots instead of a massive exact
 formula.  Read [\ref{primer-fsolve}], and ask Python to find an
 approximate solution to the equation $y=0$, where $y=x^3-3x+1$.
 It is also helpful to plot the graph (say for $-2\leq x\leq 2$) and
 see where the roots lie: read [\ref{primer-basicplot}] for help with
 this.  From either of these approaches, we see that there is
 precisely one negative root for this equation.  Use [\ref{primer-diff}]
 and [\ref{primer-subs}] to find the value of $dy/dx$ at this negative
 root. 
\end{exercise}
...
\begin{exercise}\label{ex-mobius-fix}
 ...
 where $g(x)=x$.  
 \begin{itemize}
  \item[(a)] Ask Python to solve the equation $g(x)=x$ for
   $x$.  (Remember to use the syntax in [\ref{primer-arrow}]
   when entering the definition of $g$.)  You should get two
   solutions. 
  \item[(b)] For some special value(s) of $b$ and/or $c$, Python's
   ...
   special cases.
  \item[(c)] For which value(s) of $b$ and/or $c$ are Python's two
...
\begin{exercise}\label{ex-adjusted-sin}
 ...
 \verb~exp(-x)~ and $\pi$ is \verb~Pi~).  If you ask Python to solve
 ...
 \begin{itemize}
  \item[(a)] First plot the graph of $f(x)$, say from $x=0$ to $x=10$. 
   ...
   they are? 
  \item[(b)] Enter \pcode~sc.optimize.root_scalar(f1, x0=0).root~.
   This finds a root at about $x=-0.55$, ignoring all the roots with
   $x\geq 0$ that we saw in the graph.  To find a root near $x=2$
   instead, enter \pcode~sc.optimize.root_scalar(f1, x0=2).root~.
...
\end{pcodeblock}
  \item[(c)] Now define $r(n)=n-e^{-n}/\pi-e^{-2n}/\pi^2$
   (using syntax as in [\ref{primer-seriesdef}]).  In~(a)
...
\begin{exercise}
 ...
 \begin{itemize}
  \item[(a)] To find the solutions graphically, plot the
   ...
   [\ref{primer-vertrange}].  You should see that there are many
   roots in the picture, repeating in a regular way, so there
   are actually infinitely many roots if we allow $x$ to run
   from $-\infty$ to $+\infty$.
  \item[(b)] By inspecting the graph carefully, you should be able 
   to guess the exact values of the roots.
  \item[(c)] Use \pcode~sp.solve()~ to solve the equation $\tan(\pi
   x)^2=3$ for $x$.  Python will report only two solutions, although
   we have seen that there are really infinitely many. 
  \item[(d)] You should have seen in~(b) that the solutions are all of
   the form $x=n+1/3$ or $x=m-1/3$, where $n$ and $m$ are integers. 
   To persuade sympy to find this answer, we need to use
   \pcode~sp.solveset()~ instead of \pcode~sp.solve()~, and also
   include the optional argument \pcode~domain.sp.S.Reals~ to indicate
   that we are looking for real number solutions.  This will give the
   correct answer, but in a form that is not fully simplified or
   optimally efficient.
 \end{itemize}
\end{exercise}
...
\begin{exercise}\label{ex-linear-trichotomy}
 ...
 using Python [\ref{primer-solve},\ref{primer-multisolve}].  
\end{exercise}

\chapter{Plotting 1}

Before starting each exercise, you should restart Python, either by
...
\begin{exercise}\label{ex-plot-misc}
 Plot each of the following functions [\ref{primer-basicplot}]
...
\begin{exercise}\label{ex-wiggle}
 ...
 [\ref{primer-arrow}].  Plot the graph for various ranges of
 $x$, and describe the main features that you see.  Compare
 $f(x)$ with the functions $x$, $x^2$, $x^3$ and so on, by
 plotting them together [\ref{primer-twoplots}].  Which of
...
\begin{exercise}\label{ex-mobius}
 ...
 \begin{itemize}
  \item[(a)]
   ...
   vertical range [\ref{primer-vertrange}] as well as the horizontal one.
  \item[(b)] What is the value of $x$ where $g(x)$ is discontinuous? 
   You can either work this out from the formula, or read this off
   from the graph, or use the Python command \verb~discont(g(x),x)~. 
   You can also replot the graph, skipping over the
   discontinuity, as explained in [\ref{primer-discont}].
  \item[(c)] 
   Plot the line $y=5$ along with the function $g(x)$  
   \footnote{You should use the syntax in [\ref{primer-twoplots}].  You
...
\begin{exercise}\label{ex-lhopital}
 ...
 (Remember [\ref{primer-arrow}] when entering this in Python.)
...
\begin{exercise}\label{ex-bessel}
 We now use Python to investigate the properties of a new function that
...
\end{pcodeblock}
  ...
  \item[(d)] For the size of the oscillations, try plotting $0.15x^{-1}$
   alongside $J_2(x)$, as in [\ref{primer-display}]:
\begin{pcodeblock}
...
\end{pcodeblock}
   ...
   relevant numbers, press ENTER, and Python will redraw the graph.) 
...
\begin{exercise}\label{ex-quartic}
 ...
 \[ g(a,x) =
    \frac { (x-1-\sin(a/4))(x-1-\cos(a/4))
          ...
          {1+{x}^{4}}. 
 \]
 (Remember [\ref{primer-arrow},\ref{primer-multifunc}] when entering
 this in Python.)  The object of this exercise is to describe
 the properties of $g$.  
 \begin{itemize}
  ...
  \item[(b)] Plot $g(2,x)$ and $g(4,x)$ for $x$ from $-2$ to $2$.  In
   ...
   restrict the vertical range [\ref{primer-vertrange}] to be
   from $-0.01$ to $0$, so only the part of the curve below
   the axis will be shown.  Finally, we specify
   \verb~numpoints=1000~ to make sure that Python plots the
   graph very accurately [\ref{primer-numpoints}].  
...
%\end{pcodeblock}
  ...
  \item[(d)] Plot $g(a,0.5)$ for $a$ from $-40$ to $40$.
   ...
   left of the Python window.  They should be $(3,0.9)$
   ...
   so the slope is $dg/da$, or in Python terms:
\begin{pcodeblock}
...
restarting Python, enter \verb~with(plots):~ to (re)load the
...
\begin{exercise}
 \begin{itemize}
  \item[(a)]
   ...
   as in [\ref{primer-parametric},\ref{primer-parametricbrackets}].
  \item[(b)]
  ...
  \item[(c)]
   ...
   It looks wrong because Python draws it using different scales on the
...
\begin{pcodeblock}
  ...
  y := 8*t^3-8*t;
\end{pcodeblock}
 (You must remember to remove these definitions by restarting Python
 [\ref{primer-restart}] before you go on to the next exercise.) 
 \begin{itemize}
...
\begin{exercise}
 ...
 carefully, study the reference notes, and sketch what Python would
...
Read [\ref{primer-implicitplot}] and ask Python to plot the
graph. 
...
\begin{exercise}
 ...
 picture; you can improve it as in [\ref{primer-grid}].
...
\begin{exercise}\label{ex-listplot}
 Read [\ref{primer-listplot} --- \ref{primer-listplotgen}].  Ask Python
 to draw the seven points at $x=1,\dotsc,7$ with $y$-values
 $10$, $40$, $20$, $30$, $50$, $10$ and $20$.  Do this with
 a line joining the specified points [\ref{primer-listplot}]
 and then with just the points themselves
 [\ref{primer-listplotstyle}].  Try changing the list of
...
\begin{exercise}\label{ex-plotprimes}
 \begin{itemize}
  ...
  \item[(b)] Enter \verb~seq(ithprime(i),i=1..100);~ to list the first
   hundred primes in one go [\ref{primer-seq}]. 
...
\end{pcodeblock}
   ...
   use a semicolon, then Python will print out all the coordinates of
...
\end{pcodeblock}
  ...
  RETURN at the end of each line.  Python will not attempt to carry out
...
\begin{exercise}
 \begin{itemize}
  ...
  \item[(e)] Define $f(x)=\sqrt{2\pi} x^{x+1/2} e^{-x}$
   (using [\ref{primer-arrow}]).  It is an
...
\end{pcodeblock}
 ...
 joining them [\ref{primer-listplotstyle}].  You should see that the
...
\begin{exercise}\label{ex-Li}
 ...
 \begin{itemize}
  ...
  \item[(c)] Enter \verb~Q:=(x,h)->(q(x+h)-q(x))/h;~, so that $q'(x)$
   ...
   [\ref{primer-multifunc}].  Put \verb~e:=exp(1);~ for
   convenience [\ref{primer-exp}].  Tell Python to do all numerical
   calculations to 30 digits [\ref{primer-digits}]. 
...
\begin{exercise}\label{ex-rolle}
 ...
 \begin{itemize}
  \item[(a)] How can you find the roots of $f'(x)$ by looking at the
   graph of $f(x)$? 
  \item[(b)] Define $f(x)=x^3-x$ using [\ref{primer-arrow}]. 
   ...
   Python (although it would be easy to do it by hand). 
   \begin{itemize}
    \item Ask Python [\ref{primer-roots}] to find the roots of $f(x)$. 
    \item Ask Python [\ref{primer-diff},\ref{primer-leibniz}] to find
     $f'(x)$.  (Note that \verb~f'(x)~ is \textbf{not} valid
     Python syntax.) 
    \item Ask Python to find the roots of $f'(x)$. 
  ...
  \item[(c)] Repeat part~(b) for the function
   $g(x)=\sin(x)+\sin(3x)/3$.  Python will give you six roots for
   ...
   where the rest of the roots lie.  (You could ask Python to do this
...
\begin{exercise}\label{ex-schwartzian}
 ...
 \begin{itemize}
  ...
  \item[(b)] Enter the definition into Python:
\begin{pcodeblock}
...
\end{pcodeblock}
  ...
  \item[(c)] Define $y=(ax+b)/(cx+d)$ (where $a$, $b$, $c$ and $d$ are
   constants), then ask Python to calculate and simplify $y'$, $y''$,
  ...
  \item[(d)] Now define $z=(ap^x+b)/(cp^x+d)$ (where $p$ is yet
   ...
   required to persuade Python to do the right simplifications.  Here
   is one approach that works: expand out $1/S(z)$, simplify it, and
   then take $1$ over the result. 
  \item[(e)] Now define 
   \[ T(u) = (u')^{1/2} ((u')^{-1/2})''. \] Enter this
   definition in Python (with syntax similar to part~(b)). 
...
\begin{exercise}\label{ex-fermat-spiral}
 ...
 [\ref{primer-implicitplot}] .  Remember that you need
 \verb~with(plots):~ to make this work
 [\ref{primer-withplots}].  A reasonable range is to let $x$
 and $y$ run from $-5$ to $5$, and you should ask Python to
 plot extra points [\ref{primer-grid}] to get a decent picture.
 The curve is called a \emph{Fermat spiral}.

 Now find the slope of the curve using the
 \verb+implicitdiff+ command [\ref{primer-implicitdiff}], and
 call the answer \verb+slope1+.  Python will give the answer
...
\end{pcodeblock}
 Use the \verb~subs~ command [\ref{primer-subs}] to substitute
...
\begin{exercise}\label{ex-Psi}
 Enter the following definition into Python
 [\ref{primer-seriesdef},\ref{primer-multidiff}]:
 \[ r(n) =
    \frac{d^n}{dx^n}\left(\frac{x^n\ln(x)}{n!}\right). 
 \]
 Find $r(n)$ for a reasonable range of numbers $n$, using
 the \verb~seq(...)~ command [\ref{primer-seq}].  Then
...
\begin{exercise}\label{ex-hermite}
 ...
 definition in Python [\ref{primer-seriesdef},\ref{primer-multidiff}], and then
 ...
 polynomials, which Python calls \verb~HermiteH(n,x)~.  Enter 
...
worksheet somewhere, start up Python separately, and then open the
worksheet using the File menu.  When the worksheet has started up,
click the button on the Python toolbar marked with a triple exclamation
...
calculations with Python.  When calculating integrals, use
...
This sheet deliberately gives very little help with Python syntax, and
...
\begin{exercise}\label{ex-legendre}
 ...
 \]
 Check that $q(n)=p_n$ for $n=0,\ldots,4$. (The functions $p_n$ are
 called \emph{Legendre polynomials}, and the fact that $p_n=q(n)$ is
 called \emph{Rodrigues's formula}).
\end{exercise}
...
\begin{exercise}
 ...
 functions.  In same cases, Python will write the answer in
 ...
 \]
 In some other cases, Python will just give up.  For example,
 try the following:
 ...
 Python give up?  (There is one that you can enter with just
 three characters.)
\end{exercise}
...
%  made a syntactic mistake, because $\ln(e)=1$ and Python
...
Please refer to the ``Notes on Python syntax'' as you do
these questions.

\begin{exercise}
 Enter the definitions $x_0=\sin(\theta)\sin(\phi)$,
 $x_1=\sin(\theta)\cos(\phi)$, and $x_2=\cos(\theta)$.
 Simplify $x_0^2+x_1^2+x_2^2$.
\end{exercise}
...
\begin{exercise}
 ...
 to $x=10$.  Arrange your syntax so that Python gives you an
...
\begin{exercise}
 ...
 the same graph, for $x$ from $-4$ to $4$.  Tell Python to
...
\begin{exercise}
 ...
 graph, with $x$ running from $0$ to $1$.  
\end{exercise}
...
\begin{exercise}\label{ex-ln}
 ...
 \begin{itemize}
  \item[(a)] Find [\ref{primer-multidiff}] and simplify
   $d^5y/dx^5$.  Then put $x=0$ using the \verb~subs~
   command [\ref{primer-subs}], and divide by $5!$ to get $a_5$.
...
\end{pcodeblock}
   then use the \verb~seq~ command [\ref{primer-seq}] to
...
\begin{exercise}\label{ex-sin}
 \begin{itemize}
  \item[(a)] Find the $12$th order Taylor series for
   ...
   [\ref{primer-taylor}].  Convert the answer to an ordinary
   polynomial as in [\ref{primer-taylorconv}]. Call the result $s$. 
  \item[(b)] Plot $s$ and $\sin(x)$ together
   [\ref{primer-twoplots}] for $x$ from
   $-8$ to $8$, with the vertical range also restricted
   [\ref{primer-vertrange}]  from $-10$ to $10$.  You should see that
...
\end{pcodeblock}
 You should see that Python's answer is the same as $y$
 (rearranged slightly). 
\end{exercise}
...
\begin{exercise}
 ...
 and add an option to make Python draw a more accurate picture.
\end{exercise}
...
\begin{exercise}\label{ex-expand-symmetric}
 Use Python to expand out the expression
 \[ (x+y+z)^3 - 3(x+y+z)(xy+yz+zx) + 3xyz. \]
\end{exercise}

\begin{exercise}\label{ex-expand-unshift}
 Use Python to expand out the following expressions:
 \begin{align*}
...
%    &=& x^{1/a + 1/b + 1/c},
%  \end{eqnarray*}
%  so A is true.  You can also ask Python to simplify both sides,
...
%  Python, to get $x^6+2x^3+1$ (not just $x^6+1$).  Thus, C is 
%  also false.
%  \par
...
than $d\phi_i/dx$ and $d\phi_i/dt$.  These can be entered in Python in
...
\]
Python will do this quite happily for $i=0,\dotsc,3$.  For $\phi_4$,
however, you need to help by telling Python to convert all hyperbolic
...
near the top of the Python window, which should be fairly
...
constant.  Ask Python to calculate $M_i$ for $i=0,\dotsc,3$, and you
will see that this is true.

Python is not clever enough to calculate $M_4$ symbolically.  However,
...
\end{pcodeblock}
(If we leave out \verb~method=_Gquad~ then Python will use a different
