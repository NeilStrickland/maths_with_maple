...
\DefineVerbatimEnvironment{pcodeblock}{Verbatim}{formatcom=\color{darkred}}
\newverbcommand{\pcode}{\color{darkred}}{}
...
\begin{exercise}
 Read \note{\NOTEdiff,\NOTEdiffvar}.  Then ask Python to differentiate
...
\begin{exercise}\label{ex-biquadratic-roots}
 ...
 Next, read \note{\NOTEsubs -- \NOTEsolvesubs}.  Use the \verb~subs()~
...
\begin{exercise}\label{ex-cubic-horrid}
 ...
 formula.  Read \note{\NOTEfsolve}, and ask Python to find an
...
\begin{exercise}
 ...
 \begin{itemize}
  \item[(a)] To find the solutions graphically, plot the
   ...
   \note{\NOTEvertrange}.  You should see that there are many
...
\begin{exercise}\label{ex-lhopital}
 ...
 (Remember \note{\NOTEarrow} when entering this in Python.)
...
Read \note{\NOTEimplicitplot} and ask Python to plot the
graph. 
...
\begin{exercise}\label{ex-listplot}
 Read \note{\NOTElistplot\dots\NOTElistplotgen}.  Ask Python
...
\begin{exercise}\label{ex-Li}
 ...
 \begin{itemize}
  ...
  \item[(c)] Enter \verb~Q:=(x,h)->(q(x+h)-q(x))/h;~, so that $q'(x)$
   ...
   convenience \note{\NOTEexp}.  Tell Python to do all numerical
